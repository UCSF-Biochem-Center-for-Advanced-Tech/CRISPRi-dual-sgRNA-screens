...
def getMismatchDict(table, column, trim_range=None, allowOneMismatch=True, id_column=None):
    ...
    if clash_zero or clash_one:
        ...
        print(f"{column}: {'; '.join(summary)}; marking those as multiple to avoid ambiguous mappings.")
...
def writeToCounts(fileTup):
    ...
    with gzip.open(r1file) as infile_r1, gzip.open(r2file) as infile_r2:
        ...
        for i, (r1, r2) in enumerate(zip(infile_r1, infile_r2)):
            read_idx = i // 4
            if testRun and read_idx >= testLines:
                break
            if i % 4 == 0:
                ...
                protospacer_a_r1, a_used_mismatch, a_candidates = matchBarcode(
                    ...
                    quality=qual_a,
                    low_q_threshold=LOW_QUALITY_THRESHOLD,
                )
    ...
    print(outprefix, numReads, 'reads')

    mapped_a = numReads - statsCounts['A sgRNA not mapped']
    mapped_b = numReads - statsCounts['B sgRNA not mapped']
    print('Percent A sgRNAs mapping', f"{mapped_a * 100.0 / numReads:.2f}", f'({mapped_a}/{numReads})')
    print('Percent B sgRNAs mapping', f"{mapped_b * 100.0 / numReads:.2f}", f'({mapped_b}/{numReads})')
    if has_umi:
        mapped_umi = numReads - statsCounts['UMI not mapped']
        print('Percent UMIs mapping', f"{mapped_umi * 100.0 / numReads:.2f}", f'({mapped_umi}/{numReads})')
        print(
            'Percent UMIs with multiple mappings',
            f"{statsCounts['UMI multiple mappings'] * 100.0 / numReads:.2f}",
            f"({statsCounts['UMI multiple mappings']}/{numReads})",
        )
    both_unique = statsCounts['All sgRNAs uniquely map']
    print(
        'Percent all sgRNAs mapping (both A and B uniquely)',
        f"{both_unique * 100.0 / numReads:.2f}",
        f'({both_unique}/{numReads})',
    )

    # Guard against zero uniquely mapped reads to avoid division errors on edge cases
    if both_unique == 0:
        print('Percent both A and B match 0.0 (no uniquely mapped sgRNAs)')
    else:
        both_match = statsCounts['A sgRNA and B sgRNA match']
        print(
            'Percent both A and B match',
            f"{both_match * 100.0 / both_unique:.2f}",
            f'({both_match}/{both_unique})',
        )
    both_match_total = statsCounts['A sgRNA and B sgRNA match']
    print(
        'Total percent matching and unique mapping reads',
        f"{both_match_total * 100.0 / numReads:.2f}",
        f'({both_match_total}/{numReads})',
    )

    print('A sgRNAs exact matches', statsCounts['A sgRNA exact match'])
    print('A sgRNAs via 1 mismatch', statsCounts['A sgRNA 1-mismatch match'])
    print('B sgRNAs exact matches', statsCounts['B sgRNA exact match'])
...
def read_and_validate_umi_table(path: str | None) -> pd.Series | None:
    """
    ...
    umiMismatchDict = getMismatchDict(df, 'UMI', allowOneMismatch=True)
