            ...
            with gzip.open(fileTup[2]) as infile_r3:
                for i, (r1,r2,r3) in enumerate(zip(infile_r1, infile_r2, infile_r3)):
                    if i%4 == 1:
                        ...
                        ##Modified so that I1 = UMI, R1 = sgRNA A, R2 = sgRNA B
                        protospacer_a_r1 = matchBarcode(mismatchDicts['protospacer_a_r1'], r2[:19].strip().decode("utf-8"), allowOneMismatch = False)
                        protospacer_b_r2 = matchBarcode(mismatchDicts['protospacer_b_r2'], r3[:19].strip().decode("utf-8"), allowOneMismatch = False)
                        umi = matchBarcode(mismatchDicts['umi'], r1[:8].strip().decode("utf-8"), allowOneMismatch = False)
